        ...
        results.forEach((result, idx) => {
            if(result.data.errors){
                ...
                if(result.data.data.actor.entity && result.data.data.actor.entity.relatedEntities && result.data.data.actor.entity.relatedEntities.results && result.data.data.actor.entity.relatedEntities.results.length>0){
                    ...
                    if(tags.length > 0) {
                        tags = tags.filter(el => el.key == tag_key).map(el => { return el.values });
                        if(tags && tags.length >0){
                            countAwsRegionTagPresent++;
                            entities[idx][tag_key] = [...new Set(tags.flat())];
                        } else entities[idx][tag_key] = null
                    }
                }
                else entities[idx][tag_key] = null;
            }
        });
        log(`${SYMBOLS.success} Success fetching tags: ${countInfraPresent} service with Infra Hosts and ${countAwsRegionTagPresent} services with ${tag_key} tag`);
        return entities;
...
exports.addEntitiesTag = async (entities, TAG_KEY, TAG_KEY_TO_ADD ) => {
    let promises = [];
    entities = entities.filter(el => el[TAG_KEY] != null && el[TAG_KEY].length >0);
    try{
        ...
        entities.forEach(entity => {
            promises.push(
                axios({
                    ...OPTIONS,
                    data: JSON.stringify({"query": ADD_TAG_TO_ENTITY_QUERY(entity.guid, entity[TAG_KEY], TAG_KEY_TO_ADD)})
                })
            )
        });

...
//------------------------------
const TAG_KEY = "aws.awsRegion";
const TAG_KEY_TO_ADD = "server_region";
const NR_ACCOUNT_ID = 01234;
//------------------------------

const runToAddTags = async() => {
    let entities = await fetchEntities(action = "addTags", NR_ACCOUNT_ID,TAG_KEY_TO_ADD);
    entities = await fetchInfraAWSTag(entities, TAG_KEY);
    await addEntitiesTag(entities, TAG_KEY, TAG_KEY_TO_ADD);
}
